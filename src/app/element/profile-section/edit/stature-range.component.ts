...
export class ProfileSectionEditStatureRangeComponent
  ...
  implements OnInit
{
  ...
  public minChange() {
    ...
    HttpService.api()
      .get<StatureRange[]>('keyword/max-stature-ranges', {
        params: {
          min: min,
        },
      })
    ...
    const minList$ = HttpService.api()
      ...
      .pipe(
        map((statureRanges: StatureRange[]) => {
          return (data.minList = statureRanges.map(
            (statureRange: StatureRange) => {
