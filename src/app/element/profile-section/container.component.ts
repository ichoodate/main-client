...
import { Compiler, Component, ComponentFactory, Input, OnInit, ViewChild, ViewContainerRef } from '@angular/core';
  ...
  private init$(mode: Mode): Observable<any> {
    ...
    return of([true]).pipe(
      switchMap(() => {
        ...
        const observs = [from(this.compiler.compileModuleAndAllComponentsAsync(mdlCons))];
      ...
      map(([promise]) => {
        ...
        const cmpFactory   = <ComponentFactory<any>>_.find(cmpFactories, ['componentType.name', cmpName]);

        this.component = this.container.createComponent(cmpFactory).instance;
        this.component.shared = this.shared;

        if ( mode == 'edit' ) {
          (<ProfileSectionEditComponent>this.component).profileType = this.profileType;
        }
      })
    );
  }
